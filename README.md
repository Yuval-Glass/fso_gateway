...
The system was validated using multiple stress and integration test suites:
...
C. End-to-End Integration Test (Task 20) - NEW

A full pipeline simulation was implemented to validate the entire system:

Fragmentation → Block Builder → FEC → Interleaver → Burst Loss → Deinterleaver → FEC Decode → Reassembly

Issue Discovered (Critical Bug)

During initial runs, the system produced:

Only 576 transmitted symbols instead of 768
Final interleaver window was never drained
Missing packets despite sufficient FEC redundancy
Root Cause

The last block (after block_builder_finalize_with_padding()) contained a padded source symbol with an incorrect fec_id.

Padding symbols inherited fec_id = 0
This caused a duplicate (block_id, fec_id)
One column (fec_id=K-1) was missing
Result: interleaver row never completed → window never became READY
Impact
Last interleaver window stuck at 7/8
Only 3 out of 4 windows transmitted
System silently degraded despite valid FEC conditions
Fix
...
This enforces deterministic mapping:

Source symbols → fec_id = 0..K-1
Repair symbols → fec_id = K..K+M-1
Result After Fix
Full transmission: 768 / 768 symbols
All windows drained correctly
32/32 blocks decoded successfully
100% packet recovery with exact byte match
