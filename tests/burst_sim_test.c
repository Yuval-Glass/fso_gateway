 ...
 *
 * This file turns the original single-case burst simulation into a reusable
 * diagnostic harness with three sweep modes:
 *
 ...
 *
 * Default behavior:
 *   Run all three suites sequentially.
 *
 * Optional CLI:
 *   ./build/burst_sim_test all
 *   ./build/burst_sim_test length
 *   ./build/burst_sim_test start
 *   ./build/burst_sim_test boundary
 *
 * Core invariants preserved from the previously working version:
 *   - Erasures are represented by zeroing symbol_t, so payload_len == 0
 *     is the sentinel.
 *   - The push loop skips payload_len == 0 symbols.
 *   - deinterleaver_tick(..., 0.0) is called at end-of-stream to flush
 *     partial 95/96 blocks.
                               ...
                               size_t                source_bytes)
{
    ...
    while (deinterleaver_get_ready_block(dil, &block) == 0) {
        ...
        rc = fec_decode_block(fec_dec,
                              block.symbols,
                              block.symbol_count,
                              block.symbols_per_block,
                              records[blk_id].reconstructed);

        records[blk_id].decoded_ok = (rc == 0);
                       ...
                       int               depth)
{
    ...
    if (burst_len < 0 || burst_start < 0 || burst_start + burst_len > total_symbols) {
        LOG_ERROR("[BSIM] erase_burst: invalid burst range start=%d len=%d total=%d",
                                    ...
                                    int             n)
{
    ...
    {
        LOG_ERROR("[BSIM] erase_exact_block_losses: invalid parameters block=%d losses=%d n=%d",
    ...
    for (f = 0; f < losses_to_drop; ++f) {
        ...
        if (pos < 0) {
            LOG_ERROR("[BSIM] erase_exact_block_losses: missing block=%d fec_id=%d in tx_buffer",
                      target_block, f);
            free(pos_by_fec);
            return -1;
        }
        ...
        if (tx_buffer[pos].payload_len == 0) {
            LOG_ERROR("[BSIM] erase_exact_block_losses: duplicate erase at pos=%d block=%d fec_id=%d",
                      pos, target_block, f);
            free(pos_by_fec);
            return -1;
        }
                                  ...
                                  iteration_result_t *out)
{
    ...
    out->all_blocks_pass = 0;

    if (depth < 1 || k < 2 || m < 0) {
        LOG_ERROR("[BSIM] run_iteration_internal: invalid geometry depth=%d k=%d m=%d",
                  depth, k, m);
        return -1;
    }
    ...
    dil = deinterleaver_create(dil_capacity, n, DEFAULT_SYMBOL_SIZE);
    if (il == NULL || dil == NULL) {
    ...
    for (b = 0; b < depth; ++b) {
        ...
        if (m > 0 && fec_encode_block(fec_enc, records[b].source, repair_syms, m) != 0) {
            free(repair_syms);
        ...
        for (f = 0; f < m; ++f) {
            ...
            if (rc < 0) {
                unsigned int bad_fec_id = (unsigned)repair_syms[f].fec_id;
                free(repair_syms);
    ...
    if (tx_count != total_symbols) {
        LOG_ERROR("[BSIM] drained %d instead of %d symbols", tx_count, total_symbols);
        goto cleanup;
    }

    if (erasure_mode == ERASURE_MODE_BURST) {
        erased_count = erase_burst(tx_buffer,
                                   total_symbols,
                                   burst_start,
                                   burst_len,
                                   records,
        ...
        erased_count = erase_exact_block_losses(tx_buffer,
                                                total_symbols,
                                                target_block,
                                                exact_losses,
                                                records,
...

    {
        ...
        for (i = 0; i < total_symbols; ++i) {
            ...
            if (rc < 0) {
                ...
                if (deinterleaver_get_ready_block(dil, &tmp) == 0) {
                    int tid = (int)tmp.block_id;

                    if (tid >= 0 && tid < depth && !records[tid].decoded_ok) {
                        memset(records[tid].reconstructed, 0, source_bytes);

                        rc = fec_decode_block(fec_dec,
                                              tmp.symbols,
                                              tmp.symbol_count,
                                              tmp.symbols_per_block,
                                              records[tid].reconstructed);

                        records[tid].decoded_ok = (rc == 0);

                        if (rc == 0) {
                            records[tid].data_match =
                                (memcmp(records[tid].source,
                                        records[tid].reconstructed,
                                        source_bytes) == 0);
                        }
                    }
                ...
                rc = deinterleaver_push_symbol(dil, &tx_buffer[i]);
                if (rc < 0) {
                    LOG_ERROR("[BSIM] symbol permanently rejected block=%u fec=%u",
...
static int run_iteration(int burst_len,
                         int burst_start,
                         int depth,
                         int k,
                         int m,
                         iteration_result_t *out)
{
    return run_iteration_internal(burst_len,
                                  burst_start,
                                  depth,
                                  k,
                                  m,
...
static int run_exact_boundary_iteration(int target_block,
                                        int losses_on_block,
                                        int depth,
                                        int k,
                                        int m,
                                        iteration_result_t *out)
{
    return run_iteration_internal(0,
                                  0,
                                  depth,
                                  k,
                                  m,
...
static void append_summary(summary_row_t *rows,
                           int           *row_count,
                           const char    *section,
                           const char    *label,
                           int            expected_pass,
...
static void print_summary(const summary_row_t *rows, int row_count)
{
    ...
    for (i = 0; i < row_count; ++i) {
        ...
        if (rows[i].expected_pass == rows[i].actual_pass) {
            matched++;
        } else {
            mismatches++;
        }
...
{
    static const int lengths[] = {1, 10, 32, 33, 50, 100, 200, 500, 3200, 3201};
    int failures = 0;
    int i;
    ...
    for (i = 0; i < ARRAY_LEN(lengths); ++i) {
        ...
        snprintf(label, sizeof(label), "burst_len=%d start=%d", lengths[i], start);
        append_summary(rows, row_count, "Burst Length", label, expected_pass, actual_pass);
...
static int run_burst_start_sweep(summary_row_t *rows, int *row_count)
{
    ...
    for (i = 0; i < ARRAY_LEN(starts); ++i) {
        ...
        snprintf(label, sizeof(label), "burst_len=%d start=%d", burst_len, starts[i]);
...
static int run_exact_boundary_tests(summary_row_t *rows, int *row_count)
{
    ...
    for (i = 0; i < ARRAY_LEN(losses); ++i) {
        ...
        if (run_exact_boundary_iteration(target_block,
                                         losses[i],
                                         DEFAULT_DEPTH,
                                         DEFAULT_K,
                                         DEFAULT_M,
        ...
        snprintf(label, sizeof(label), "block=%d losses=%d", target_block, losses[i]);
        append_summary(rows, row_count, "Exact Boundary", label, expected_pass, actual_pass);
